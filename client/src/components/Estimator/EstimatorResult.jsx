...
import { EstimateBreakdownChart } from '../Chart';
import { downloadEstimatePDF } from '../../services/pdfApi';

const EstimatorResult = ({ estimatedCost, formData, resetCalculator }) => {
  // Simulación de token y estimateId para demo
  const token = localStorage.getItem('token');
  const estimateId = estimatedCost?.id || 'demo-id';

  const handleDownloadPDF = async () => {
    try {
      await downloadEstimatePDF({ estimateId, token });
    } catch (e) {
      alert('Error al descargar PDF: ' + (e.message || 'Error desconocido'));
    }
  };
          ...
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl p-8 shadow-2xl">
            <div className="text-center mb-8">
              ...
              <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-2">
                <span className="text-lg text-purple-200 font-semibold">Tiempo estimado:</span>
                <span className="text-2xl text-white font-bold">{formData.timeline} semanas</span>
              </div>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="mt-6">
                <EstimateBreakdownChart estimatedCost={estimatedCost} />
              </div>
            </div>
            {/* Action Buttons */}
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={handleDownloadPDF} className="flex-1 flex items-center justify-center space-x-2 px-6 py-4 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold hover:shadow-lg transform hover:scale-105 transition-all">
